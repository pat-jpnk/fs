...
void _search(struct searchItem *item, struct options options)
{
    ...
    char *buffer = malloc((size * sizeof(char))); // void pointer returned by malloc is
    ...
    else
    {
        fprintf(stderr, KCYN "%s not read, file empty\n" KRESET,
                item->path); // triggers on empty file
    }
...
void _replace(struct searchItem *item, struct options options)
{
    ...
    {
        new_len =
            ((buffer_index) + (((int) st_occurrence_count) * (((int) replacement_term_len) - ((int) search_term_len))));
    ...
    {
        fprintf(stdout, KRED "%.*s - no search term occurrences found\n" KRESET, (int) sizeof(item->path),
                item->path); // %.*s -> convenient to print char array
        free(buffer);
        return;
    }
